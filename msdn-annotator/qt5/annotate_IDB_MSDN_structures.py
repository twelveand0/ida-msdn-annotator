...
class MSDNAnnotationDialog(QtWidgets.QDialog):
    ...
    def on_ok_button(self):
        ...
        if not os.path.exists(msdnpath):
            ...
            ret = QtWidgets.QMessageBox.warning(self, 'MSDN Info Not Found', 'The file %s was not found in the specified MSDN Data Directory' % IDB_MSDN_Annotator.MSDN_STRUCTURE_INFO_FILE, QtWidgets.QMessageBox.Ok)
    ...
    def populate_form(self):
        ...
        self.chkStructuresAnnotate = QtWidgets.QCheckBox("Annotate structure names"
                                                    " (see note)")
        layout1.addWidget(self.chkStructuresAnnotate)
        self.chkStrucsRepeatable = QtWidgets.QCheckBox("Use repeatable comments "
                                                   "for structure name "
                                                   "annotations")
        layout1.addWidget(self.chkStrucsRepeatable)

        # Members
        self.chkMembersAnnotate = QtWidgets.QCheckBox("Annotate structure "
                                                    "members (see note)")
        layout1.addWidget(self.chkMembersAnnotate)

        # Constants
        self.chkConstantsImport = QtWidgets.QCheckBox("Rename constants (no preview)")
