...
"C:\Program Files (x86)\IDA 6.8\tilib.exe" \
"C:\Program Files (x86)\IDA 6.8\til\pc"


Authors: Binchang, Liu
Copyright 2016 VARAS, IIE of CAS

This work is a updated version from Moritz Raabe and William Ballenthin 's work \
-Copyright 2014:
...
# @bc.
def parse_new_style_structure(file, content, const_enum):
    # Check for > not for /> because some docs are broken
    api_types = re.findall(
        "<MSHelp:Attr Name=\"APIType\" Value=\"(.*?)\"[ /]*>", content)
    # "UserDefined" TODO
    if api_types in ([], ["Schema"], ["UserDefined"],
                     ["MOFDef"], ["NA"], ["LibDef"]):
        return None
    
    if api_types not in ([], ["COM"], ["DllExport"], ["HeaderDef"]):
        g_logger.debug("API Type: ", ' '.join(api_types))
    
    # structure name    
    structure_names = re.findall(
        "<MSHelp:Attr Name=\"APIName\" Value=\"(.*?)\"[ /]*>", content)
    
    if structure_names:
        # indicates which standard enum(s) IDA knows for this member
        constant_enum = {}
        ...
        if m:
            ...
            g_logger.debug("Error: Could not retrieve data type description"
                             " from file %s" % file)
            return None
        
        # members
        m = re.search("<P CLASS=\"clsRef\">Members</P>"
                      "<BLOCKQUOTE>(.*?)</BLOCKQUOTE>", content)
        if m:
            member_names = re.findall("<DT><B>(.*?)</B>", m.group(1))
            descriptions = [strip_html(string) for string in
                            re.findall("<DD>(.*?)</DD>", m.group(1))]
            members = zip(member_names, descriptions)
        else:
            m = re.search("<h3>Members</h3><dl>(.*?)</dl><h3>", content)
            if m:
                # all member names
                member_names = re.findall("<dt><b>(.*?)</b>", m.group(1))
                member_names = [
                    mem_name.replace("<b>", "") for mem_name in member_names]
                
                # all member descriptions    
                descriptions = re.findall("<dd>(.*?)</dd>", m.group(1))
                stripped_descriptions = []
                for descr in descriptions:
                    descr_m = re.match(r'<p>(.*?)</p><table>', descr)
                    if descr_m:
                        stripped_descriptions.append(strip_html(descr_m.group(1)))
                    else:
                        stripped_descriptions.append(strip_html(descr))
                members = zip(member_names, stripped_descriptions)
                
                # Find constants and store them in dictionary
                constant_data = {}
                for i in range(0, len(member_names)):
                    
                    # not every member has descriptions
                    if len(descriptions) != len(member_names):
                        constant_data = None
                        break
                    ...
                    constant_names = re.findall(
                        "<tr><td>(.*?)</td>", descriptions[i])
                    constant_names = [strip_html(unicode(c, 'utf-8'))
                                      .encode('utf-8') for c in constant_names]
                    constant_descriptions = re.findall(
                        "</td><td>(.*?)</td></tr>", descriptions[i])
                    constant_descriptions = [strip_html(unicode(string, 'utf-8'))
                                      .encode('utf-8') for string in constant_descriptions]
                    ...
                    # Only add entries for members with constant data
                    if constant_names:
                        constant_data[member_names[i]] = zip(
                    ...
                    if constant_names and found_enums:
                        ...
                        constant_enum[member_names[i]] = ','.join(
                            found_enums)
                        g_logger.debug('final constant data from database',
                                       member_names[i], constant_enum)
                    
            else:
                # without 'Members', it's not a structure definition
                return None
            
        dll_names_str = ""        
        for dll_name in dll_names:
            dll_names_str += dll_name + ","
            
        dll_names_str = dll_names_str[:-1]
            
        return_list = []    
        for structure_name in structure_names:
            g_logger.debug("Found structure %s!%s (new style)" %
                           (dll_name.lower(), structure_name))
            return_list.append((dll_names_str.lower(), structure_name,
                               description, members, constant_data,
                               constant_enum))
        return return_list
...
def parse_new_style(file, content, const_enum):
    ...
    if function_names:
        ...
        # @bc. add
        m = re.search("<h3>Return Value</h3><p>(.*?)</p><h3>", content)
        if m:
            return_value = strip_html(m.group(1))
        else:
...
def parse_file(type, file, const_enum):
    ...
    if content.find("ph:apidata") != -1:
        ...
        if type == "function":
            ...
            return parse_new_style_structure(file, content, const_enum)
        else:
            return None
    else:
        return None
...
# @bc.
def to_xml_structure(results):
    xml_string = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>"
    xml_string = xml_string + "<msdn>\n"
    xml_string = xml_string + "<structures>\n"
    ...
    for (dll_names, structure_name, description, members, constant_data,
         constant_enum) in results:
        xml_string = xml_string + "\t<structure>\n"
        xml_string = xml_string + "\t\t<name>%s</name>\n" % structure_name
        
        for dll_name in dll_names.split(','):
            xml_string = xml_string + "\t\t<dll>%s</dll>\n" % dll_name
        
        xml_string = xml_string + \
            "\t\t<description>%s</description>\n" % description

        xml_string = xml_string + "\t\t<members>\n"

        for (member_name, member_description) in members:
            xml_string = xml_string + "\t\t\t<member>\n"
            xml_string = xml_string + \
                "\t\t\t\t<name>%s</name>\n" % member_name
            xml_string = xml_string + \
                "\t\t\t\t<description>%s</description>\n" \
            ...
            # add information about identified constants for this argument
            if constant_data and member_name in constant_data.keys():
                member_name, structure_name
                if member_name in constant_enum.keys():
                    xml_string = xml_string + "\t\t\t\t<constants enums=\"" + \
                        constant_enum[member_name] + "\">\n"
                else:
                    xml_string = xml_string + "\t\t\t\t<constants>\n"
                for (constant_name, constant_description) in constant_data[member_name]:
                    xml_string = xml_string + "\t\t\t\t\t<constant>\n"
                    xml_string = xml_string + "\t\t\t\t\t\t<name>%s</name>\n" \
                        % constant_name.replace('<b>', '').replace('</b>', '').replace('<i>', '').replace('</i>', '').replace('<a>', '').replace('</a>', '')
                    xml_string = xml_string + \
...
def parse_files(type, msdn_directory, tilib_exe, til_dir):
    """
    ...
    for root, dirs, files in os.walk(msdn_directory):
        for file in files:
            ...
            if file.endswith('htm'):
                file_counter += 1
                result = parse_file(type, os.path.join(root, file), const_enum)
                if result:
...
def show_usage():
    print 'Usage:',
    print sys.argv[0] + ' -t structure|function <path to msdn> <path to tilib.exe> <til directory>'
...
def main():
    ...
    if not (sys.argv[2] == "structure" or sys.argv[2] == "function"):
        print "The type must be structure or function"
        show_usage()
        sys.exit(1)
    ...
    type = sys.argv[2]
    msdn_directory = sys.argv[3]
    tilib_exe = os.path.abspath(sys.argv[4])
    til_dir = os.path.abspath(sys.argv[5])

    (file_counter, results) = parse_files(type, msdn_directory, tilib_exe, til_dir)
    results = sum(results, [])

    print "Parsed %d files" % file_counter
    print "Extracted information about %d objects" % len(results)
    ...
    if type == "function":
        ...
        xml_file.write(to_xml_structure(results))
